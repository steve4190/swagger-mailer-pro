...
import { Send, Paperclip, X, FileText, File, Image, AlertTriangle, Loader, Upload, Users, Settings, Plus, QrCode, Terminal, User } from 'lucide-react';
import { BulkEmailData } from '../App';
...
interface BulkEmailComposerProps {
  onSendBulkEmail: (bulkEmailData: BulkEmailData) => Promise<{ success: boolean; error?: string }>;
  isConfigured: boolean;
  defaultFromName?: string;
}

export default function BulkEmailComposer({ onSendBulkEmail, isConfigured, defaultFromName }: BulkEmailComposerProps) {
  ...
  React.useEffect(() => {
    if (defaultFromName && !bulkEmailData.fromName) {
      setBulkEmailData(prev => ({ ...prev, fromName: defaultFromName }));
    }
  }, [defaultFromName]);
  ...
  const handleRecipientsChange = (value: string) => {
    ...
    setBulkEmailData(prev => ({ ...prev, recipients: emails }));
  };
  ...
  const handleCSVUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      setBulkEmailData(prev => ({ ...prev, recipients: uniqueEmails }));
    };
  ...
  const handleSend = async () => {
    ...
    if (bulkEmailData.recipients.length === 0) {
      setSendResult({ success: false, message: 'Target list required' });
      return;
    }
    ...
    if (!bulkEmailData.textContent?.trim() && !bulkEmailData.htmlContent?.trim()) {
    ...
    try {
      const result = await onSendBulkEmail(bulkEmailData);
      if (result.success) {
        setSendResult({ success: true, message: `Bulk operation successful! ${bulkEmailData.recipients.length} targets reached.` });
        // Reset form
        setBulkEmailData({
          recipients: [],
          subject: '',
        ...
        setSendResult({ success: false, message: result.error || 'Bulk operation failed' });
      }
    } catch (error) {
      setSendResult({ 
        success: false, 
    ...
    <div className="max-w-4xl">
      <div className="mb-8">
        <div className="flex items-center mb-4">
          ...
          <h2 className="text-2xl font-bold text-green-400 tracking-wider glitch">BULK OPERATIONS</h2>
        </div>
        <p className="text-green-600 tracking-wide">Execute mass email campaigns with advanced targeting and batching</p>
      </div>
      ...
      <div className="space-y-6">
        <div>
          ...
          <input
            type="text"
            value={bulkEmailData.fromName || ''}
            onChange={(e) => setBulkEmailData(prev => ({ ...prev, fromName: e.target.value }))}
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="block text-sm font-bold text-green-400 tracking-wider">
              TARGET LIST * ({bulkEmailData.recipients.length} targets loaded)
            </label>
            <div className="flex space-x-2">
              ...
              <button
                ...
                onClick={() => {
                  setRecipientText('');
                  setBulkEmailData(prev => ({ ...prev, recipients: [] }));
                }}
                className="flex items-center px-3 py-2 text-sm bg-red-900/50 text-red-400 rounded-lg hover:bg-red-900/70 transition-colors border border-red-500/30 font-semibold tracking-wider"
              >
                CLEAR ALL
              </button>
            </div>
          ...
          <p className="mt-1 text-xs text-green-600">
            Separate targets with commas or new lines. Duplicates auto-filtered.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="relative">
              <input
                ...
                name="sendMethod"
                value="bcc"
                checked={bulkEmailData.sendMethod === 'bcc'}
                onChange={(e) => setBulkEmailData(prev => ({ ...prev, sendMethod: e.target.value as 'bcc' | 'individual' }))}
                className="sr-only"
              />
              <div className={`p-4 border-2 rounded-lg cursor-pointer transition-colors ${
                bulkEmailData.sendMethod === 'bcc'
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-bold text-green-400 tracking-wider">BCC METHOD</h3>
                    <p className="text-sm text-green-600">Single transmission with hidden recipients (stealth mode)</p>
                  </div>
                </div>
              </div>
            </label>
            ...
            <label className="relative">
              <input
                ...
                name="sendMethod"
                value="individual"
                checked={bulkEmailData.sendMethod === 'individual'}
                onChange={(e) => setBulkEmailData(prev => ({ ...prev, sendMethod: e.target.value as 'bcc' | 'individual' }))}
                className="sr-only"
              />
              <div className={`p-4 border-2 rounded-lg cursor-pointer transition-colors ${
                bulkEmailData.sendMethod === 'individual'
                  ? 'border-green-400 bg-green-900/30'
                  : 'border-green-500/30 hover:border-green-400/50'
              }`}>
                <div className="flex items-center">
                  <Send className="w-5 h-5 mr-3 text-green-400" />
                  <div>
                    <h3 className="font-bold text-green-400 tracking-wider">INDIVIDUAL METHOD</h3>
                    <p className="text-sm text-green-600">Separate transmission per target (personalized)</p>
                  </div>
                </div>
              </div>
            </label>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="flex items-center text-sm text-cyan-400 hover:text-cyan-300 font-semibold tracking-wider"
              >
            ...
            {showAdvanced && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-900/50 rounded-lg border border-green-500/30">
                <div>
                  <label className="block text-sm font-bold text-green-400 mb-2 tracking-wider">
                    BATCH SIZE
                  </label>
                  <input
                    ...
                    min="1"
                    max="100"
                    value={bulkEmailData.batchSize}
                    onChange={(e) => setBulkEmailData(prev => ({ ...prev, batchSize: parseInt(e.target.value) }))}
                    className="w-full px-3 py-2 bg-gray-900 border border-green-500/50 rounded-lg focus:ring-2 focus:ring-green-400 focus:border-transparent text-green-400 font-mono"
                  />
                  <p className="mt-1 text-xs text-green-600">Targets per batch execution</p>
                </div>
                <div>
                  <label className="block text-sm font-bold text-green-400 mb-2 tracking-wider">
                    DELAY (ms)
                  ...
                  <input
                    ...
                    max="10000"
                    step="100"
                    value={bulkEmailData.delayBetweenBatches}
                    onChange={(e) => setBulkEmailData(prev => ({ ...prev, delayBetweenBatches: parseInt(e.target.value) }))}
                    className="w-full px-3 py-2 bg-gray-900 border border-green-500/50 rounded-lg focus:ring-2 focus:ring-green-400 focus:border-transparent text-green-400 font-mono"
                  />
                  <p className="mt-1 text-xs text-green-600">Delay between batch executions</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div>
          ...
          {emailType === 'rich' ? (
            <RichTextEditor
              ...
              onChange={(value) => setBulkEmailData(prev => ({ ...prev, htmlContent: value }))}
              placeholder="Initialize bulk message payload..."
              height="400px"
            ...
            <textarea
              value={bulkEmailData.textContent || ''}
              onChange={(e) => setBulkEmailData(prev => ({ ...prev, textContent: e.target.value }))}
              rows={12}
              ...
              placeholder="Enter plain text bulk message payload..."
            />
          ) : (
            <div className="space-y-3">
              <textarea
                value={bulkEmailData.htmlContent || ''}
                onChange={(e) => setBulkEmailData(prev => ({ ...prev, htmlContent: e.target.value }))}
                rows={12}
                className="w-full px-3 py-2 bg-gray-900 border border-green-500/50 rounded-lg focus:ring-2 focus:ring-green-400 focus:border-transparent font-mono text-sm text-green-400"
                placeholder="Enter HTML bulk message payload..."
              />
              <div className="bg-gray-900 border border-green-500/50 rounded-lg p-4">
                ...
                <div 
                  ...
                  dangerouslySetInnerHTML={{ __html: bulkEmailData.htmlContent || '<p class="text-gray-400">HTML preview will render here...</p>' }}
                />
              </div>
            </div>
          )}
        ...
        <div>
          ...
          {bulkEmailData.attachments && bulkEmailData.attachments.length > 0 && (
            <div className="space-y-2">
              {bulkEmailData.attachments.map((file, index) => (
        ...
        <div className="flex justify-between items-center">
          <div className="text-sm text-green-600 font-semibold tracking-wider">
            READY TO DEPLOY: <span className="text-green-400">{bulkEmailData.recipients.length}</span> TARGETS
            {bulkEmailData.sendMethod === 'individual' && bulkEmailData.recipients.length > 0 && (
              <span className="ml-2">
                ({Math.ceil(bulkEmailData.recipients.length / (bulkEmailData.batchSize || 10))} BATCHES)
              </span>
            )}
          </div>
          <button
            onClick={handleSend}
            disabled={isSending || !isConfigured || bulkEmailData.recipients.length === 0}
          ...
          >
            ...
            {isSending ? 'EXECUTING...' : 'EXECUTE BULK OPERATION'}
          </button>
        </div>
      </div>
    </div>
